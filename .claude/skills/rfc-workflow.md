...
**默认流程**（无需 grill-me）：
1. 写 `design.md` 初稿（架构图 + 设计决策表 + 伪代码 + 文件变更清单）
2. 自审设计决策表，确认所有 D1-Dn 已闭合
3. 如决策点明确无争议，直接进入 Phase 2
4. 输出：design.md v1（决策闭合，可进入审查）

**grill-me 深度访谈**（仅在用户明确要求时使用）：
1. 调 `/grill-me`，从最阻塞的决策开始，一次一个问题，深度优先遍历
2. 每个决策收敛后立即写进 design.md，不等到最后
3. 输出：design.md v2（所有决策树闭合，无未决分支）
...
## Phase 3: 实现（并行 Sonnet agent 文件级隔离）

**模型**: 全部使用 `sonnet`。**模式**: 并行 agent，文件级隔离（任意两个 agent 不修改同一文件）。

**硬性要求**：agent 输出代码必须 **零编译错误、零 import 断裂、零未定义引用**。接手后 `typecheck + build` 直接通过，禁止"见光死"代码（写完就报错）。
...
## Phase 4: 验证（并行审查 + 自动化测试 + Playwright）
...
### 4b. 并行 Sonnet 审查（必做）
派发 2-3 路 `sonnet` agent 并行审查：
- **功能正确性**：对照 design.md 检查实现是否正确、完整
- **回归安全**：对比修改前后的组件树/API 行为，确认无破坏
- **代码质量**：未使用的 import、重复逻辑、可简化模式

每个 agent 输出问题清单（🔴/🟡/🟢），修复 🔴+🟡 后重新跑自动化门禁。

### 4c. Playwright 深度体验（涉及 UI 的 RFC 必做）
- 入口页交互链路完整（输入 → 创建 → 跳转）
- 游戏页核心操作流畅（header/sidebar/narrative/input）
- 所有页面截图 + snapshot 比对
- 截图写入 `execution.md`
...
**铁律：RFC 完成后必须立即归档，不允许挂着"正在进行"状态开始新 RFC。**
