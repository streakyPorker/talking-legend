...
> - 验收类 → Playwright 深度体验（多页面截图 + 交互验证）。**涉及前端路由/渲染/浏览器API的变更，必须 Playwright 端到端验证，typecheck+build 通过不等于能跑。**
...
- **`backend/`** — NestJS 11 + better-sqlite3, global `/api` prefix. Module breakdown:
  - **Feature modules** (`game`, `npc`, `world`, `storyline`) each follow Controller → Service → Repository. Requests validated by zod schemas (`*.schema.ts`) via a global `ZodValidationPipe`; `AllExceptionsFilter` returns structured errors; `LoggingInterceptor` logs requests. `game/` includes `ContextProvider` (RFC-016: data injection layer reading DB → creating ContextModule instances → registered in Map for `ContextBuilder.build()`), `NarrativeService` (narrative persistence), SSE streaming endpoints (`POST /:id/action/stream`), and save/load (RFC-017): DB-file snapshot — `saveGame` copies `data/talking-legend.db` → `data/saves/slot_N.db` and records metadata in the `saves` table; `POST saves/:slot/load` copies the file back and returns the `game_id` for the frontend redirect.
  - **`db/`** — `DbModule.forRoot()` is `@Global`: opens SQLite in WAL mode, runs idempotent single-file schema from `db/migrate.ts` (`CREATE TABLE IF NOT EXISTS`, no versioning — 11 tables: games/worlds/npcs/npc_memories/players/player_quests/storylines/llm_logs/travel_log/saves/game_events), and exports repositories + the `DB_INSTANCE` token. Multi-write ops run inside `db.transaction(...)`; turn bumps use optimistic concurrency. Repository specs use `createInMemoryDb()` (`:memory:`) — no Nest bootstrap needed.
