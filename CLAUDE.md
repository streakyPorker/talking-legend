...
1. 后端 RFC：**NestJS 必须成功启动**，路由映射正常，API 返回有效响应
2. 前端 RFC：**Vite 必须成功启动**，页面可访问，关键交互可用
3. execution.md 必须记录**实际启动日志**作为验收证据（不是 checkmark，是真实输出）
4. 禁止标记 RFC 为"已完成"如果应用不能正常启动
